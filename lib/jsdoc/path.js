...
function prefixReducer(previousPath, current) {
    ...
    currentPath = path.resolve(runtime.pwd, current).split(path.sep) || [];
...
exports.commonPrefix = function(paths) {
    ...
    if (paths.length === 1) {
        prefix = path.resolve(runtime.pwd, paths[0]);
...
exports.getResourcePath = function(filepath, filename) {
    ...
    [path.dirname(runtime.opts.configure || ''), runtime.pwd, runtime.dirname].forEach(function(_path) {
        if (!result && _path) {
